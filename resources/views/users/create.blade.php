...
@section('title', 'Tambah Akun - ' . config('app.name'))
@section('pageTitle', 'Tambah Akun')
@section('pageSubtitle', 'Buat akun pemilik atau karyawan')

@section('content')
    <form method="POST" action="{{ route('users.store') }}" class="max-w-5xl">
        @csrf
        ...
        <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input name="email" type="email" value="{{ old('email') }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-100">
                </div>
                ...
                <div>
                    ...
                    <select name="role" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-100">
                        ...
                        <option value="owner" @selected(old('role') === 'owner')>Pemilik Toko</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Password</label>
                    <div class="relative mt-1">
                        <input id="user_password_create" name="password" type="password" required class="w-full rounded-xl border border-slate-200 px-3 py-2 pr-11 text-sm outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-100">
                        <button type="button" data-password-toggle="user_password_create" class="absolute inset-y-0 right-2 grid w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700" aria-label="Tampilkan password">
